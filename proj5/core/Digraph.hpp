...
struct DigraphVertex
{
    ...
    DigraphVertex(VertexInfo input_Vinfo) :vinfo(input_Vinfo)
    {}
};
...
public:
    ...
    std::map<int, int> findShortestPaths(
        int startVertex,
        std::function<double(const EdgeInfo&)> edgevalFunc) const;
...
private:
    ...
    typedef typename list<DigraphEdge<EdgeInfo>>::const_iterator edgeitr;
    typedef typename map<int,DigraphVertex<VertexInfo,EdgeInfo>>::const_iterator graphitr;
...
std::vector<int> Digraph<VertexInfo, EdgeInfo>::vertices() const
{
    ...
    for (auto it=graphmap.begin();
            it!=graphmap.end(); ++it) {
        verVec.push_back(it->first);
    }
...
std::vector<std::pair<int, int>> Digraph<VertexInfo, EdgeInfo>::edges() const
{
    ...
    for(auto iter=graphmap.begin();
            iter!=graphmap.end();iter++) {
        for(auto it = iter->second.edges.begin();
                it!=iter->second.edges.end();it++) {
            edges.push_back(make_pair(it->fromVertex,it->toVertex));
        }
    }
...
{
    if (graphmap.count(vertex)==0) {
        throw DigraphException("Doesn't containe this vertex");
    }

    std::vector<std::pair<int, int>> edges;
    for(auto it = graphmap.find(vertex)->second.edges.begin();
            it!=graphmap.find(vertex)->second.edges.end();it++) {
        edges.push_back(std::make_pair(it->fromVertex,it->toVertex));
    }
...
{
    if(graphmap.count(fromVertex)==0 || graphmap.count(toVertex)==0)
    {
        throw DigraphException("Invalid");
    }
    for( auto it=graphmap.at(fromVertex).edges.begin();
        it!=graphmap.at(fromVertex).edges.end();it++)
    {
...
void Digraph<VertexInfo, EdgeInfo>::addVertex(int vertex, const VertexInfo& vinfo)
{
    for(auto it=graphmap.begin(); it!=graphmap.end();it++) {
        if(it->first == vertex) {
            throw DigraphException("The vertex exis");
        }
    }

    DigraphVertex<VertexInfo,EdgeInfo> d = DigraphVertex<VertexInfo,EdgeInfo>(vinfo);

    graphmap.insert(std::pair<int,DigraphVertex<VertexInfo,EdgeInfo>>(vertex,d));
}
...
{
    if (graphmap.count(fromVertex) == 0 
            || graphmap.count(toVertex) == 0) {
        throw DigraphException("verteice doesn't exist");
    }
    graphmap.at(fromVertex).edges.push_back(DigraphEdge<EdgeInfo>{fromVertex, toVertex, einfo});
...
void Digraph<VertexInfo, EdgeInfo>::removeVertex(int vertex)
{
    ...
    for(auto it=graphmap.begin();it!=graphmap.end();it++)
    {
        for(auto it_2=it->second.edges.begin();
        it_2!=it->second.edges.end();it_2++)
        {
            if(it_2->toVertex==vertex)
            {
                it->second.edges.erase(it_2);
            }
        }
    }
...
void Digraph<VertexInfo, EdgeInfo>::removeEdge(int fromVertex, int toVertex)
{
    ...
    for(auto it=graphmap.begin();it!=graphmap.end();it++)
    {
        ...
        it_2!=it->second.edges.end();it_2++)
        {
            if(it_2->toVertex==toVertex && it_2->fromVertex==fromVertex)
            {
                it_2=it->second.edges.erase(it_2);
            }
        }
    }
}
...
int Digraph<VertexInfo, EdgeInfo>::edgeCount(int vertex) const
{
   ...
   return graphmap.at(vertex).edges.size();
}
...
{
    auto p = graphmap.begin();
    auto q = graphmap.end();
    while(p!=q)
    {
        if(vertexCount() != edgeCount(p->first))
            return false;
        p++;
    }
    return true;
